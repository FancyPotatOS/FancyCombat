

# Minecraft Fancy Combat Datapack

<em>Created by FancyPotatOS</em>


More shields and combat mechanics for tools!

Also adds a wand and camoflauge outfit!

## Tool Changes

### Pickaxe
When hit, gives Blindness for 1 second

This isn't enough to cause visual changes, however it prevents players from starting a sprint!

### Shovel
When hit, gives Weakness II for 1 second

### Hoe
Increased range, attack speed, and attack damage

### Shears
When hit, reduces block interaction range by 1.5 blocks for 1.5 seconds

### Brush
When hit, reduces entity interaction range by 1.5 blocks for 2.5 seconds

## New items

### Buckler
- 150 Durability
- No startup delay
- Reduces all attack damage by 3 hearts
- 1:3 disable time
- Is damaged 1:1 with damage taken

![Buckler Recipe](img/buckler_recipe.png)

### Tower Shield
- 450 Durability
- Half a second startup delay
- 90% damage reduction
- 1:5 disable time
- Is damaged 1:1 with damage taken
- Reduces movement speed by ~37%
- Reduces entity interaction range by half a block

![Tower Shield Recipe](img/tower_shield_recipe.png)

## Camoflauge

This armor allow you to hide yourself as blocks - simply crouch with your back against a block and you'll take on its appearance. While you're wearing the entire set, you'll get permanent invisibility to hide your nametag.

To reset the appearance, press Crouch+Sprint+Jump at the same time

The strength is between gold and iron.

![camo_recipe](img/camo_recipe.gif)

![camo_animated](img/camo_animated.gif)

## Wands

Wands are special items that gives you access to projectiles or abilities that you normally couldn't use. Casting them costs durability.

The 'Untrimmed Wand' ![untrimmed_wand](img/wand/base.gif) can be used with a trim in your offhand to set the ability. They can have Unbreaking and Mending applied to it using an anvil.

They are found in the following chests
- Ancient City (1:7)
- Hoglin Stables (1:2)
- Bastion Treasure Room (1:2)
- Desert Pyramid (1:4)
- End City Treasure (1:5)
- Jungle Temple (1:2)
- Pillager Outpost (1:7)
- Stronghold Library (1:2)
- Woodland Mansion (1:2)

### Bolt ![bolt](img/wand/bolt.png)
<hr />

Cost: 8\
Cooldown: 1s

Changes the appearance of nearby creatures (Like horses, tropical fish, cats, etc.)

### Coast ![coast](img/wand/coast.png)
<hr />

#### Not Sneaking
Cost: 8\
Cooldown: 30s

Freezes the water around nearby creatures ![ice](img/ice.png)

#### Sneaking
Cost: 64 and a Heart of the Sea ![HotS](img/heart_of_the_sea.png)\
Cooldown: 20 minutes (For this ability)

Starts a Lightning Storm ![lightning_storm](img/thunderstorm.png)

### Dune ![dune](img/wand/dune.png)
<hr />

Cost: 32 and TNT ![tnt](img/tnt_side.png)\
Cooldown: None

Places a TNT trap when stepped on. Placer cannot trigger it for 5 seconds

If buried, the trap will activate when the block is mined instead

### Eye ![eye](img/wand/eye.png)
<hr />

Cost: 32\
Cooldown: 30s

Shoots an Enderdragon Fireball ![enderdragon_fireball](img/dragon_fireball.png)

### Flow ![flow](img/wand/flow.png)
<hr />

Cost: 16 and either a Lingering Potion ![lingering_potion](img/lingering_potion.gif) or Arrow ![arrow](img/arrow.gif)\
Cooldown: 15s

Places an Ominous Item Spawner over nearby entities

### Host ![host](img/wand/host.png)
<hr />

Cost: 1 and a Bundle ![bundle](img/bundle.gif)\
Cooldown: None

Places the bundle into sand or gravel, suspiciously! ![suspicious_blocks](img/suspicious_blocks.gif)

_Remember that the items will break if the suspicious blocks fall. Don't put anything important in there!_

### Raiser ![raiser](img/wand/raiser.png)
<hr />

Cost: 16
Cooldown: 40s

Attracts items within 15 blocks towards you for 30 seconds

### Rib ![rib](img/wand/rib.png)
<hr />

#### Not Sneaking
Cost: 8\
Cooldown: 10s

Shoots a Wither Skull ![wither_skull](img/wither_skull.png)

#### Sneaking
Cost: 24\
Cooldown: 20s

Shoots a Blue Wither Skull ![blue_wither_skull](img/blue_wither_skull.png)

### Sentry ![sentry](img/wand/sentry.png)
<hr />

Cost: 24\
Cooldown: 30s

Summons a forcefield that reflects all projectiles. Effect lasts for 5-10 seconds

### Shaper ![shaper](img/wand/shaper.png)
<hr />

Cost: 8 and Scaffolding\
Cooldown: None

Places a falling block trap when stepped on. Placer cannot trigger it for 5 seconds

The block must be mineable by a stone pickaxe

### Silence ![silence](img/wand/silence.png)
<hr />

Cost: 48\
Cooldown: 30s (10s if missed)

Continue using for 2 seconds to launch a Sonic Boom up to 25 blocks towards the entity you're looking at

### Snout ![snout](img/wand/snout.png)
<hr />

#### Not Sneaking
Cost: 16\
Cooldown: 10s

Shoots Fireballs ![small_fireball](img/small_fireball.png) like a Blaze

#### Sneaking
Cost: 24\
Cooldown: 20s

Shoots a Fireball ![small_fireball](img/small_fireball.png) like a Ghast

### Spire ![spire](img/wand/spire.png)
<hr />

Cost: 16\
Cooldown: 10s

Shoots a Shulker Bullet ![shulker_bullet](img/shulker_bullet.png) that targets the nearest entity, with the attacker as the lowest priority

### Tide ![tide](img/wand/tide.png)
<hr />

Cost: 8\
Cooldown: 30s

Boosts the player's water-related attributes for 15-30 seconds:
- Submerged Mining Speed (+40%)
- Underwater Movement (+80%)
- No Burn Time
- Oxygen Bonus (80% chance not to consume air)

### Vex ![vex](img/wand/vex.png)
<hr />

Cost: 24\
Cooldown: 10s

Performs an Evoker Fang ![fang](img/fangs.png) attack up to 16 blocks

### Ward ![ward](img/wand/ward.png)
<hr />

Cost: 24\
Cooldown: 15s

Senses grounded movement for 15-30 seconds. Produces sculk sounds and vibration effects ![vibration](img/vibration.gif) towards all nearby motion. Any creatures that made a sound do not trigger this again for 3 seconds

### Wayfinder ![wayfinder](img/wand/wayfinder.png)
<hr />

Cost: 1\
Cooldown: 1s

Shoots a Llama Spit ![spit](img/spit.png)

### Wild ![wild](img/wand/wild.png)
<hr />

Cost: 4 and an arrow ![trap_arrows](img/trap_arrow.gif)\
Cooldown: None

Places an arrow trap when stepped on. Placer cannot trigger it for 5 seconds

If buried, the trap will activate when the block is mined instead

## Credits

**In this datapack comes packaged the [Iris pathfinding datapack](https://github.com/Aeldrion/Iris) by Aeldrion**

**In this datapack comes packaged the [BlockState datapack](https://github.com/Triton365/BlockState) by Triton365**
